    ...
    <q-dialog v-model="confirm">
      <q-card>
        <q-card-section class="row items-center">
          <span class="q-ml-sm">You really want to cancel this book?</span>
        </q-card-section>
...
//const rows = ref([...originalRows])
    ...
    const getTurs = () => {
      axios({
        url: "http://localhost:4000/api/book/getboughtbooks", 
        method: "GET"
      }).then((res) => {
    ...
    const deleteTur = () =>{
      axios({
        url: "http://localhost:4000/api/book/cancelbook/" + Id.value, 
        method: "DELETE"
      }).then((res) => {
        if(res.status == 200){
          ...
          confirm1.value = true
          TurCanceled.value = "Book canceled successfully"
        }
      })
      .catch((err) => {
        TurCanceled.value = "you can't cancel this Book"
      })
    }
    const deletereq = (id) => {
      confirm.value = true;
